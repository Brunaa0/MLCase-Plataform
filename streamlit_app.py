...
class CustomPDF(FPDF):
    def __init__(self, orientation='P', unit='mm', format='A4'):
        ...
        # Garantir que o cabeçalho seja exibido
        self.alias_nb_pages()
    ...
    def header(self):
        ...
        if self.logo_path and os.path.exists(self.logo_path):
            try:
                ...
                self.set_font('Arial', 'B', 12)
                self.cell(0, 10, "Logo não disponível", 0, 1, 'C')
        else:
            self.set_font('Arial', 'B', 12)
            self.cell(0, 10, "Logo não disponível", 0, 1, 'C')
    ...
    def footer(self):
        ...
        current_date = datetime.now().strftime('%d/%m/%Y')
        # Conteúdo do rodapé
        self.cell(0, 10, f'{current_date} - Página {self.page_no()}/{self.alias_nb_pages()}  |  Autora da Plataforma: Bruna Sousa', 0, 0, 'C')
...
def gerar_relatorio_pdf(comparison_df, best_model, session_state):
    ...
    # Inicialização do PDF com fonte padrão embutida
    pdf = CustomPDF()
    pdf.set_auto_page_break(auto=True, margin=15)
    
    # Adicionar a primeira página - isto irá acionar o método header()
    pdf.add_page()
