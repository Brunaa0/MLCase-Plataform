...
class CustomPDF(FPDF):
    ...
    def footer(self):
        ...
        self.set_font('DejaVu Sans', 'I', 10)
        # Data atual
...
def data_summary():
    ...
    plt.figure(figsize=(8, 6))
    sns.heatmap(correlation_matrix, annot=True, cmap="coolwarm", fmt=".4f", cbar=True, square=True)
    plt.title('Matriz de Correlação das Variáveis', fontsize=14, fontweight='bold', fontname='DejaVu Sans')
    st.pyplot(plt)
...
def gerar_relatorio_pdf(comparison_df, best_model, session_state):
    ...
    pdf.set_font("DejaVu Sans", style="B", size=10)
    pdf.set_fill_color(200, 220, 255)  # Cor de fundo do cabeçalho
