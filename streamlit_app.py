...
class MLCaseModelReportGenerator:
    ...
    def create_bar_chart(self, data, labels, title):
        ...
        bars = plt.bar(labels, data, color=['#9ACD32', '#006400'])  # Verde claro e verde escuro
        ...
        for bar in bars:
            yval = bar.get_height()
            plt.text(bar.get_x() + bar.get_width() / 2, yval + 0.01, round(yval, 4), ha='center', va='bottom', color='white')
...
def gerar_relatorio_pdf(comparison_df, best_model, session_state):
    ...
    pdf.cell(0, 10, txt="Relatório Final dos Modelos Treinados", ln=True, align="C")
    pdf.ln(10)
    
    # Adicionar informações sobre o modelo, conjunto de dados, e melhores métricas
    model_type = session_state.get('model_type', 'Indefinido')
    pdf.set_font("Arial", style="B", size=12)
    pdf.cell(60, 10, "Tipo de Modelo:", ln=False)
    pdf.set_font("Arial", size=12)
    pdf.cell(0, 10, model_type, ln=True)
    
    selected_model_name = session_state.get('selected_model_name', 'Não Selecionado')
    pdf.set_font("Arial", style="B", size=12)
    pdf.cell(60, 10, "Modelo Selecionado:", ln=False)
    pdf.set_font("Arial", size=12)
    pdf.cell(0, 10, selected_model_name, ln=True)
    
    pdf.set_font("Arial", style="B", size=12)
    pdf.cell(60, 10, "Melhor Modelo:", ln=False)
    pdf.set_font("Arial", size=12)
    pdf.cell(0, 10, best_model, ln=True)
    pdf.ln(10)
